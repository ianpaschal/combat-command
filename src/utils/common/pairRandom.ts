...
export const pairRandom = <T extends RankingFactorKey>(
  ...
  while(unpairedCompetitors.length > 0) {
    const competitor = unpairedCompetitors[0];
    const opponentIndex = Math.floor(Math.random() * (unpairedCompetitors.length - 1)) + 1;
    const opponent = unpairedCompetitors[opponentIndex];
