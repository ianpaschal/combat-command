...
import styles from './LeagueLogo.module.scss';

type LeagueLogoShape = 'round' | 'square' | 'hexagon' | 'shield';

export type LeagueLogoWrapper = {
  shape: LeagueLogoShape;
...
export interface LeagueLogoProps {
  ...
  url: string;
  shadow?: boolean;
  wrapper?: LeagueLogoWrapper
}

const shapes: Record<LeagueLogoShape, { outer: string, inner: string }> = {
  round: {
...
export const LeagueLogo = ({
  ...
  shadow = true,
  wrapper,
}: LeagueLogoProps): JSX.Element => {
  const id = useId();
  const clipId = `${id}-clip`;
  const clipShapeId = `${id}-clip-shape`;

  return (
    <div className={clsx(styles.LeagueLogo, className)}>
